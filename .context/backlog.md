...
### Deferred from 004 PR Review

#### Multi-Session Channel Routing (RI-04)

In `src/slack/handlers/steer.rs`, `store_from_slack` picks the first active session from the database regardless of the originating Slack channel. When multiple workspaces are active in different channels, a steering message posted in channel A could be delivered to a session running in channel B. This is acceptable for the single-workspace-per-server model in 004, but must be addressed in 005 when multi-workspace support and workspace-to-channel mapping are introduced. The fix should filter sessions by `channel_id` (or the future `workspace_id`) before selecting the primary session.
...
- Need an /arc subcommand to fetch a listof all skills available in the project workspace.

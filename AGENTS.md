...
Single-workspace Rust project with two binaries (agent-intercom server and agent-intercom-ctl CLI). Feature branches, conventional commits, CI gates (fmt -> clippy -> test -> audit). Backlogit for task tracking, Slack for operator approval and remote steering.
...
`#![forbid(unsafe_code)]` enforced at the workspace level; no unsafe code anywhere. All clippy pedantic warnings are hard errors. `unwrap_used` and `expect_used` are denied at the workspace level. Zero warnings allowed.
All errors must be propagated via `Result<T, AppError>`. The AppError enum captures all error categories. No panics in production code.
...
| **Skill Discovery** — Find available capabilities on demand by keyword | `scripts/search.ps1` / `scripts/search.sh` (when Primitive 6 installed) | Ad-hoc capability lookup |
...
| `review` (agent) | `review` skill | Converted from agent to skill; dispatches persona subagents |
| `plan-review` (agent) | `plan-review` skill | Converted from agent to skill; dispatches persona subagents |

...
- [ ] T001 Create `src/driver/mod.rs` with `AgentDriver` trait definition, `AgentEvent` enum, and module doc comments
...
- [ ] T005 [P] Write unit test for `ProtocolMode` serde round-trip in `tests/unit/session_model_tests.rs`
- [ ] T006 [P] Write unit test for `AppError::Acp` display format in `tests/unit/error_tests.rs`
- [ ] T007 [P] Write unit test for `AgentEvent` enum construction and field access in `tests/unit/driver_trait_tests.rs`
...
- [ ] T013 [P] Add `set_thread_ts(session_id, thread_ts)` update method to `src/persistence/session_repo.rs`
- [ ] T014 Create new index `idx_session_channel` and `idx_session_channel_thread` in `src/persistence/schema.rs`
...
- [ ] T015 [P] [US1] Write unit test for `Mode` enum CLI parsing (mcp default, acp explicit, invalid) in `tests/unit/cli_tests.rs` — covers S001, S002, S003, S006
- [ ] T016 [P] [US1] Write unit test for ACP config validation (missing host_cli) in `tests/unit/config_tests.rs` — covers S004, S005

### Implementation

- [ ] T017 [US1] Add `Mode` enum (`Mcp`, `Acp`) with `ValueEnum` derive to `src/main.rs` CLI struct
- [ ] T018 [US1] Add `--mode` flag to `Cli` struct in `src/main.rs` with `default_value_t = Mode::Mcp`
- [ ] T019 [US1] Add ACP config validation in `src/config.rs` — validate `host_cli` is non-empty and exists when ACP mode selected
- [ ] T020 [US1] Branch `run()` in `src/main.rs` on mode: MCP path unchanged, ACP path skips MCP transport startup
- [ ] T021 [US1] Verify MCP mode regression — all 9 tools visible and functional with no changes (S007)
...
- [ ] T022 [P] [US2] Write contract test for `McpDriver::resolve_clearance` approved/rejected in `tests/contract/driver_contract_tests.rs` — covers S008, S009
- [ ] T023 [P] [US2] Write contract test for `resolve_clearance` with unknown request_id in `tests/contract/driver_contract_tests.rs` — covers S012
- [ ] T024 [P] [US2] Write unit test for driver `interrupt` on terminated session (idempotent) in `tests/unit/driver_trait_tests.rs` — covers S016
- [ ] T025 [P] [US2] Write concurrent resolution test (two requests resolved simultaneously) in `tests/unit/driver_trait_tests.rs` — covers S017

### Implementation

- [ ] T026 [US2] Define `AgentDriver` trait in `src/driver/mod.rs` with 5 methods: `resolve_clearance`, `send_prompt`, `interrupt`, `resolve_prompt`, `resolve_wait`
- [ ] T027 [US2] Implement `McpDriver` in `src/driver/mcp_driver.rs` — wraps existing `PendingApprovals`, `PendingPrompts`, `PendingWaits` oneshot maps
- [ ] T028 [US2] Wire `McpDriver` as `Arc<dyn AgentDriver>` into `AppState` in `src/mcp/handler.rs`
- [ ] T029 [US2] Refactor Slack approval handler in `src/slack/handlers/` to call `driver.resolve_clearance()` instead of directly accessing oneshot maps
- [ ] T030 [US2] Refactor Slack prompt handler to call `driver.resolve_prompt()` instead of directly accessing oneshot maps
- [ ] T031 [US2] Refactor Slack wait handler to call `driver.resolve_wait()` instead of directly accessing oneshot maps

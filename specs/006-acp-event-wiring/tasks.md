...
- [ ] T009 [P] [US1] Write contract tests in tests/contract/acp_event_contract.rs for ClearanceRequested handler pipeline with mock Slack/DB: standard flow with all fields and low risk (S001), None diff→empty diff_content (S002), high risk level (S003), critical risk level (S004), missing session→warn log+discard event+no side effects (S005), Slack unavailable→persist to DB+register with driver+skip Slack post (S006), DB persistence failure→warn+continue+driver still registered (S007), empty description string (S008), large diff >100KB→stored in full+Slack blocks truncated (S009)

### Implementation for User Story 1

- [ ] T010 [US1] Implement `AgentEvent::ClearanceRequested` match arm in src/main.rs `run_acp_event_consumer` (replacing current no-op log at line ~769): (1) look up session via `SessionRepo::find_by_id(session_id)`, if not found emit `warn!` with session_id and `continue` to next event; (2) emit `info!` tracing span with session_id, event type "ClearanceRequested", and request_id (FR-014); (3) parse `risk_level` string to `RiskLevel` enum, defaulting to `RiskLevel::Low` for unrecognized values — use case-sensitive matching per FR-011; (4) resolve file_path within session workspace via `path_safety` and compute SHA-256 content hash (use `sha2::Sha256`), set to `"new_file"` if file doesn't exist or path is rejected; (5) construct `ApprovalRequest::new(session_id, title, Some(description), diff.unwrap_or_default(), file_path, risk_level, original_hash)` and persist via `ApprovalRepo::create(&approval)` — on DB failure emit `warn!` and skip remaining steps for this event (do NOT register with driver if DB fails, per UF-03); (6) register with `AcpDriver::register_clearance(session_id, approval.id)` using the **DB-generated approval ID** (not event.request_id) so that Slack button action_ids match driver lookup keys; (7) build Slack blocks via `crate::slack::blocks::build_approval_blocks(...)` + `crate::slack::blocks::approval_buttons(&approval.id)`; (8) if Slack configured, post via `SlackService::post_message_direct(channel, blocks, thread_ts)` and save returned `slack_ts` on approval record; if Slack not configured, emit `warn!` and skip; (9) wrap every fallible operation in match/if-let with `warn!` + continue on error (D3: log and continue)
- [ ] T011 [US1] Run quality gates — verify all US1 unit tests (S018–S023, S030–S035, S055) and contract tests (S001–S009) pass: `cargo check && cargo clippy -- -D warnings && cargo fmt --all -- --check && cargo test`
...
- [ ] T014 [US2] Implement `AgentEvent::PromptForwarded` match arm in src/main.rs `run_acp_event_consumer` (replacing current no-op log at line ~788): (1) look up session via `SessionRepo::find_by_id(session_id)`, if not found emit `warn!` with session_id and `continue`; (2) call `AcpDriver::register_prompt_request(session_id, prompt_id)` via state.driver; (3) parse `prompt_type` string to `PromptType` enum, defaulting to `PromptType::Continuation` for unrecognized values (research RQ-5); (4) construct `ContinuationPrompt::new(session_id, prompt_text, prompt_type, None, None)` with elapsed_seconds=None and actions_taken=None, persist via `PromptRepo::create(&prompt)`; (5) build Slack blocks via `crate::slack::blocks::build_prompt_blocks(prompt_text, prompt_type, None, None, prompt_id)`; (6) if Slack configured, enqueue via `SlackService::enqueue(channel, blocks, thread_ts)` (Design Decision D2: enqueue for prompts); if Slack not configured, emit `warn!` and skip; (7) wrap all fallible operations in warn-and-continue error handling (Design Decision D3)
- [ ] T015 [US2] Run quality gates — verify all US2 unit tests (S024–S029, S056) and contract tests (S010–S017) pass: `cargo check && cargo clippy -- -D warnings && cargo fmt --all -- --check && cargo test`
...
- **US2 (Phase 4)**: Depends on Phase 2 completion; logically independent of US1 but modifies same src/main.rs — recommended sequential after US1

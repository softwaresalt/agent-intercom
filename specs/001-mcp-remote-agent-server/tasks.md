...
- [X] T116 Write unit tests for policy loader in `tests/unit/policy_tests.rs`: valid policy file parsing, malformed file fallback to deny-all, commands not in global allowlist rejected (FR-011), missing policy file returns deny-all
...
- [X] T063 [US6] Implement policy hot-reload via `notify` file watcher in `src/policy/watcher.rs`: watch `.monocoque/settings.json` for each active workspace_root using `notify::RecommendedWatcher`; on change event, reload policy via loader; register/unregister watchers as sessions start/terminate (FR-010)

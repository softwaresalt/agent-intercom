...
' -f owner="softwaresalt" -f repo="agent-intercom" -F pr=<pr_number>
```
...
```bash
gh api repos/softwaresalt/agent-intercom/commits/<head_sha>/check-runs \
...
```bash
gh api repos/softwaresalt/agent-intercom/check-runs/<check_run_id>/annotations \

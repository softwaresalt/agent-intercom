...
- Rust stable, edition 2021 (Tier 1 + Tier 2); TypeScript/Node.js (Tier 3 Playwright scripts) + `slack-morphism` 2.17 (existing), `serde_json` (existing), `tokio` (existing); Playwright (new, Tier 3 only — Node.js external tool) (008-slack-ui-testing)
- SQLite via sqlx (in-memory for tests — existing) (008-slack-ui-testing)
...
## Recent Changes
- 008-slack-ui-testing: Added Rust stable, edition 2021 (Tier 1 + Tier 2); TypeScript/Node.js (Tier 3 Playwright scripts) + `slack-morphism` 2.17 (existing), `serde_json` (existing), `tokio` (existing); Playwright (new, Tier 3 only — Node.js external tool)

...
1. Inspect the stash through backlog-native operations instead of manually scanning files when the tool surface can answer the question.
2. Group entries by urgency, clarity, and expected value:
   * deliberate now
...
5. When the `agent-intercom` and `backlogit` capability packs are both installed, make any remote choice broadcast self-contained: include each candidate item's ID, priority, kind or type, a one-line summary, and the recommended ordering so the operator can choose from the intercom channel alone.
...
* The request involves unknowns that require hands-on exploration, prototyping, benchmarking, or evaluating external tools
...
Invoke the **deliberate** skill for items that need scope definition, option comparison, or open-question resolution. Require a durable deliberation artifact before moving into planning.

If a stash entry is already well-formed and does not need a decision conversation, record that choice and continue directly to planning.

#### Step 2b: Invoke Spike Skill (Investigative Path)

Invoke the **spike** skill for items that require hands-on exploration. The skill produces a findings artifact that can be promoted to the planning pipeline.
...
Present the decomposition summary:
...
* Ready backlog handoff (the input to the Ship agent)
...
| Candidate set | `broadcast` | `info` | `[STAGE] Candidates: {item_ids_with_priority_kind_and_summary}` |
| Selection request | `broadcast` | `info` | `[STAGE] Priority order: {ordered_list_with_rationale}. Awaiting operator selection.` |
...
the candidate-set and priority-order broadcasts are a hardening requirement, not
optional narration. Include enough detail for a remote operator to choose the
next item without reopening the chat transcript.
...
* Always let the operator make the final decision; recommend but do not dictate
* Never promote to plan without the operator's explicit confirmation

...
**Postcondition** (harness-architect): The task has `harness-ready` label, the harness compiles (`cargo check --all-targets`), and all tests fail with expected failure markers.
...
**Precondition**: `cargo check --all-targets` exits 0 AND `cargo test` exits non-zero with expected failure markers in the output for every test function.
...
|---------|--------------|------------------|----------------------------|
| 1.0.0   | 2026-04-14   | Initial registry | Generated by autoharness   |
| 1.1.0   | 2026-04-14   | Added P-006      | Plan hardening gate enforcement |

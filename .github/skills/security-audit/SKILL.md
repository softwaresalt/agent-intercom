...
| `the GitHub Copilot CLI and VS Code agentic config ruleset (.github/**, .vscode/**)` | Per-environment config rule table |
| `the Rust OWASP detection pattern set (unsafe blocks, unchecked deserialization, string-built SQL via sqlx, std::process::Command injection, path traversal, and credential exposure)` | Language-specific OWASP detection patterns |
...
Rules from `the GitHub Copilot CLI and VS Code agentic config ruleset (.github/**, .vscode/**)`:
...
Scan source files matching `src/**` (or the specified path) using `the Rust OWASP detection pattern set (unsafe blocks, unchecked deserialization, string-built SQL via sqlx, std::process::Command injection, path traversal, and credential exposure)`:

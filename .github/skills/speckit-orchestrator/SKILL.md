...
input:
  properties:
    ...
    start-stage:
      ...
      enum: [specify, clarify, plan, behavior, tasks, analyze]
    skip-clarify:
      type: boolean
...
The feature specification pipeline consists of 6 stages executed in strict order:

```
Stage 1: Specify  ──→  Stage 2: Clarify (optional)  ──→  Stage 3: Plan
                                                                │
                                                                ▼
                  Stage 4: Behavior  ──→  Stage 5: Tasks  ──→  Stage 6: Analyze
                                                                       │
                                                                       ▼
                                                                  Handoff to
                                                             build-orchestrator
...
Context compaction occurs after Stages 1, 3, 4, and 6 — the stages that produce the largest artifacts and consume the most context window space.
...
**Full mode** (default):
* Build the stage queue: `[specify, clarify, plan, behavior, tasks, analyze]`
* If `skip-clarify` is true, remove `clarify` from the queue.

**From mode**:
* Validate that `start-stage` is provided and is a valid stage name.
* Build the stage queue starting from `start-stage` through `analyze`.
...
**Purpose**: Perform read-only cross-artifact consistency analysis across spec, plan, tasks, and scenarios.
**Dispatch**: Read the analyze agent and follow its complete workflow. This stage identifies inconsistencies, duplications, and underspecified items.
**Context to pass**: None beyond what the agent discovers from the feature directory.
...
Context compaction is performed after Stages 1, 3, 4, and 6 by reading and following the compact-context skill at `.github/skills/compact-context/SKILL.md`.
...
5. **Handing off** — clearly separating the specification lifecycle from the implementation lifecycle (build-orchestrator).
